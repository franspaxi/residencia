 ...
 */
@Entity
@Table(name = "comp_despachos", catalog = "sysventas", schema = "")
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "CompDespachos.findAll", query = "SELECT c FROM CompDespachos c")
    , @NamedQuery(name = "CompDespachos.findByIdProbDespachos", query = "SELECT c FROM CompDespachos c WHERE c.idProbDespachos = :idProbDespachos")
    , @NamedQuery(name = "CompDespachos.findByDireccion", query = "SELECT c FROM CompDespachos c WHERE c.direccion = :direccion")})
public class CompDespachos implements Serializable {
    ...
    @Column(name = "idProbDespachos", nullable = false)
