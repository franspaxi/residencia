 ...
 */
@Entity
@Table(name = "vent_despachos", catalog = "sysventas", schema = "")
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "VentDespachos.findAll", query = "SELECT v FROM VentDespachos v")
    , @NamedQuery(name = "VentDespachos.findByIdDespachos", query = "SELECT v FROM VentDespachos v WHERE v.idDespachos = :idDespachos")
    , @NamedQuery(name = "VentDespachos.findByDireccion", query = "SELECT v FROM VentDespachos v WHERE v.direccion = :direccion")})
public class VentDespachos implements Serializable {
    ...
    @Column(name = "idDespachos", nullable = false)
